...
static char kActionSheetDelegaterKey;

#pragma mark Custom delegate

@interface SCActionSheetDelegater : NSObject <UIActionSheetDelegate>

@property (nonatomic, strong) NSMutableDictionary* handlers;

@end

@implementation SCActionSheetDelegater
...
- (id)initWithTitle:(NSString *)title {
	return [self initWithTitle:title delegate:self.delegater cancelButtonTitle:nil destructiveButtonTitle:nil otherButtonTitles:nil];
}

- (SCActionSheetDelegater*)delegater {
    SCActionSheetDelegater* aDelegater = [self associatedValueForKey:&kActionSheetDelegaterKey];
    if (!aDelegater) {
        aDelegater = [[SCActionSheetDelegater alloc] init];
        [self associateValue:aDelegater withKey:&kActionSheetDelegaterKey];
    }
    return aDelegater;
}
